...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)                  % number of trials
        ...
        %------------------------------------------------------------------
        [OUT(:,i), OPTIONS] = WSCT_X_tutorial(MDP(:,i),OPTIONS);
...
for m = 1:size(MDP,1)
    ...
    % initialise posteriors over policies and action
    %----------------------------------------------------------------------
    P{m}  = zeros([Nu(m,:),1]);
    un{m} = zeros(Np(m),1);
    u{m}  = zeros(Np(m),1);
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            [MDP(m).mdp(t), OPTIONS] = WSCT_X_tutorial(mdp);
        ...
        if ~HMM || T == t
                ...
                for i = 1:Ni            % iterate belief updates
                    ...
                    for j = 1:S         % loop over future time points
                        ...
                        for f = 1:Nf(m)
                            ...
                            %----------------------------------------------
                            sx = full(x{m,f}(:,j,k));   % q(s^(f)_(tau=j)_(policy=k))
                            qL = zeros(Ns(m,f),1);      % 
                            v  = zeros(Ns(m,f),1);
                            ...
                            if dF > exp(-8) || i > 4
                                ...
                                if j <= t
                                    ...
                                    qL = spm_log(qL(:));     % ln(A.o.s)
                                end
                                
                                % entropy
                                %------------------------------------------
                                qx  = spm_log(sx);      % ln(q(s^(f)_(tau=j)_(policy=k)))
                                
                                % emprical priors (forward messages)
                                %------------------------------------------
                                if j < 2
                                    px = spm_log(D{m,f});   % ln(D)
                                    v  = v + px + qL - qx;  % state prediction error (timestep 1)
                                else
                                    px = spm_log(sB{m,f}(:,:,V{m}(j - 1,k,f))*x{m,f}(:,j - 1,k));   % ln(B_(tau-1)_policy*s_(tau-1)_policy)
                                    v  = v + px + qL - qx;  % state prediction error (timestep > 1)
                                end
                                ...
                                %------------------------------------------
                                if j < R
                                    px = spm_log(rB{m,f}(:,:,V{m}(j    ,k,f))*x{m,f}(:,j + 1,k));   % ln(B_(tau+1)_policy*s_(tau+1)_policy)
                                    v  = v + px + qL - qx;          % voltage getting updated ( v = ln(s_policy_tau) )
                                end
            ...
            if Np(m) > 1
                ...
                for i = 1:Ni
                    ...
                    qu = spm_softmax(qE{m}(p{m}) + w{m}(t)*Q(p{m}) + F(p{m}));  % pi = sigma(lnE - F - gamma.G)
